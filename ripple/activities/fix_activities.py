...
async def create_fix_pr_activity(payload: dict) -> dict:
    ...
    for c in field_changes:
        fn = c["field_name"]
        if fn not in seen:
            seen[fn] = c.get("change_type", "")
    field_summary = ", ".join(
        f"{fn} {ct.lower().replace('_', ' ')}" if ct else fn
        for fn, ct in list(seen.items())[:3]
    )
    title = f"fix: update consumer for {producer_service} contract change ({field_summary})"
...
def _build_pr_body(
    ...
    producer_link = f"[{producer_service} PR]({producer_pr_url})" if producer_pr_url else f"`{producer_service}`"

    # Deduplicate changes by field_name, keep first occurrence per field
    ...
    changes_md = "\n".join(
        ...
        for c in seen_fields.values()
    )

    # Deduplicate impacts by file_path:line
    seen_impacts: dict[str, dict] = {}
    for i in breaking_impacts:
        key = f"{i.get('file_path', '')}:{i.get('line', '')}"
        if key not in seen_impacts:
            seen_impacts[key] = i
    impacts_md = "\n".join(
        f"- `{i.get('file_path', '?')}:{i.get('line', '?')}` — {(i.get('explanation', '') or '')[:100]}"
        for i in seen_impacts.values()
    )

    return f"""## Ripple Auto-Fix

`{producer_service}` changed {len(seen_fields)} contract field(s) · see {producer_link}
...
**Before merging**
- [ ] Verify the fix is semantically correct
- [ ] Run the test suite
- [ ] Merge after {producer_link} is merged

---
*Ripple · `/learn <rule>` to add architecture rules*"""
...
async def comment_fix_prs_on_producer_activity(payload: dict) -> dict:
    ...
    lines = ["**Ripple raised fix PRs for affected consumers:**", ""]
    for r in successful:
