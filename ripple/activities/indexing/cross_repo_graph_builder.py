...
- Generic type parameters: array<T> vs array, List<String> vs List, Optional<X> vs X etc. are NOT type disagreements. Do not emit TYPE_CHANGED purely because one side specifies a generic parameter and the other does not.
- HTTP error response fields: fields inside response.4xx or response.5xx paths (e.g. response.500.status, response.404.status) are HTTP infrastructure — an integer status there is an HTTP code, not a domain enum. Never compare these to domain enums like OrderStatus, PaymentStatus, etc.
- Regex UNIT hints (pence_to_pounds, divide_by_100): only trust these when the field is clearly monetary by name or context (amount, price, fee, cost, balance, revenue, total, subtotal). Do NOT apply them to point scores, ratings, counts, phone numbers, identifiers, or fields where the /100 or *100 appears in nearby business logic (e.g. score normalisation) rather than directly on the field value itself.
- NULLABLE_CHANGED safe direction: do NOT flag when consumer models a field as Optional/nullable but the producer declares it required. That is defensive coding and creates no runtime risk. Only flag when consumer accesses the field with NO null guard and the producer declares it nullable — that is the direction that will cause a NullPointerException/crash."""
