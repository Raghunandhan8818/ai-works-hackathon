...
_TEST_PATH_RE = re.compile(
    r"(^|/)(test|tests|__tests__|__test__|spec|specs|__mocks__)(/|$)"
    r"|[._-](test|spec)\.(js|jsx|ts|tsx|py|java|go|rb|kt)$"
    r"|(Test|Tests|Spec|IT)(\.java|\.kt)$"
    r"|^test_.*\.py$|.*_test\.py$",
    re.IGNORECASE,
)
    ...
    for hit in hits:
        ...
        usages.append(
            FieldUsage(
                ...
                operations=ops,
                is_test=is_test_file(hit.rel_path),
                source_context="\n".join(context)[:2000],
            )
        )
