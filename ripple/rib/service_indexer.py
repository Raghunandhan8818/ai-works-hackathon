...
from ripple.rib.enricher.belief_enricher import enrich_belief, infer_operations_batch
...
async def index_consumer(
    ...
    # Clear stale usages before re-indexing so removed consumer callsites don't persist
    cleared = store.clear_usages_for_service(service_name)
    if cleared:
        logger.info("index_consumer cleared %d stale usages service=%s", cleared, service_name)
    ...
    # ── Infer semantic operations from source_context (batched LLM) ──
    if all_usages:
        enriched_usages = infer_operations_batch(all_usages)
        for usage in enriched_usages:
            if usage.operations:
                store.upsert_usage(usage)
        counts["usages_with_ops"] = sum(1 for u in enriched_usages if u.operations)
        logger.info(
            "index_consumer ops_inference service=%s usages_with_ops=%d",
            service_name, counts["usages_with_ops"],
        )
    ...
    # Contradictory beliefs reveal semantic drift that no schema diff can detect.
    # Collect all qualifying (fqn, beliefs) pairs, then run one concurrent batch.
    cross_coros = []
    cross_fqns = []
    for fqn in {u.field_fqn for u in all_usages}:
        try:
            blast = store.get_blast_radius(fqn)
            beliefs = [e.belief for e in blast.consumers if e.belief is not None]
        except Exception:
            continue
        if len(beliefs) >= 2:
            cross_coros.append(detect_cross_consumer_drift(fqn, beliefs))
            cross_fqns.append(fqn)

    if cross_coros:
        cross_results = await _gather_limited(cross_coros)
        for fqn, events in zip(cross_fqns, cross_results):
            for event in (events or []):
                store.upsert_drift_event(event)
                logger.warning(
                    "CROSS-CONSUMER DRIFT field=%s severity=%s: %s",
                    fqn, event.severity.value, event.drift_explanation,
                )
